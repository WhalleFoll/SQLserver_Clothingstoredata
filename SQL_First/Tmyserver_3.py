...
class StartPage:
    def __init__(self, parent_window):
        ...
        self.window.title('服装店数据管理系统')
        self.window.iconbitmap('dingdan3.ico')  # 更改窗口图标
        self.window.geometry('500x410+500+100')  # 这里的乘是小x，第一个参数表示窗口的长，第二个表示宽，第三个表示的距离屏幕左边界的距离，第三个为距离上边界的距离

        label = Label(self.window, text="服装店数据管理系统", font=("微软雅黑", 20))
        label.pack(pady=70)  # pady=100 这个label距离窗口上边界的距离，这里设置为100刚好居中
        ...
        Button(self.window, text="管理员登陆", font=tkFont.Font(size=16), command=lambda: AdminPage(self.window), width=30,
               height=2,
               fg='green', bg='white', activebackground='darkslategray',
        ...
        Button(self.window, text="数据库初始化", font=tkFont.Font(size=16), command=self.Initialization, width=30, height=2,
               fg='green', bg='white', activebackground='darkslategray', activeforeground='white').pack()
    ...
    def Initialization(self):
        db = pymssql.connect('YANPENG', 'sa', 'Peng.0921', 'master')  # 服务器名,账户,密码,数据库名
        cursor = db.cursor()
        print('建立数据库中......')
        sql = """
		-- 创建EMIS数据库
		...
		(
			...
			FILENAME='H:\\python\\My_SQL\\EMIS_data.mdf', --主文件文件名
		...
		(
			...
			filename='H:\\python\\My_SQL\\EMIS_log.ldf',--日志文件屋里文件名
			SIZE=5MB,--日志文件初始大小
			filegrowth=0 --启动自动增长
		)
		"""
    ...
    # 创建数据表
    def jianbiao(self):
        db = pymssql.connect('YANPENG', 'sa', 'Peng.0921', 'EMIS')  # 服务器名,账户,密码,数据库名
        cursor = db.cursor()
        print('建立数据表中......')
        sql = """
		---创建管理员数据表
		...
		---向学生表中插入数据
		INSERT INTO t_goods VALUES ('1000000', '卫衣1', '99', '1000');
		INSERT INTO t_goods VALUES ('1000001', '卫衣2', '200', '1500');
		INSERT INTO t_goods VALUES ('1000002', '外套1', '120', '1000');
		INSERT INTO t_goods VALUES ('1000003', '外套2', '200', '900');
		INSERT INTO t_goods VALUES ('1000004', '裤子1', '120', '2000');
		INSERT INTO t_goods VALUES ('1000005', '裤子2', '300', '10000');
		INSERT INTO t_goods VALUES ('1000006', '毛衣', '80', '20000');
		...
		dizhi nvarchar(255) DEFAULT NULL
		);

		---向卖家信息数据表中插入数据
		INSERT INTO t_buyer VALUES ('08300205', '小红', '1', '天津市');
		INSERT INTO t_buyer VALUES ('08300206', '小明', '1', '北京市');
		INSERT INTO t_buyer VALUES ('08300207', '小芬', '1', '安阳市');
		INSERT INTO t_buyer VALUES ('08080929', '小爱', '1', '武汉市');
		...
		vic nvarchar(255) DEFAULT NULL,
		buy_money varchar(255) DEFAULT NULL,
		PRIMARY KEY (SNO)
		);

		---向订单信息表中插入数据
		INSERT INTO t_order VALUES ('1000000', '08300205', '2018-7', '是', '99');
		INSERT INTO t_order VALUES ('1000002', '08300206', '2019-5', '是', '120');
		INSERT INTO t_order VALUES ('1000003', '08300207', '2020-9', '是', '200');
		INSERT INTO t_order VALUES ('1000004', '08080929', '2021-12', '是', '120');
		"""
    ...
    def create_chufaqi(self):
        ...
        cursor.close()  # 关闭游标
        db.close()  # 关闭数据库连接
        self.create_chufaqi2()

#--------------------------------------
    # 创建触发器2
    def create_chufaqi2(self):
        db = pymssql.connect('YANPENG', 'sa', 'Peng.0921', 'EMIS')  # 服务器名,账户,密码,数据库名
        cursor = db.cursor()
        print('建立触发器中......')
        sql = """
        ...
        if((select vic from inserted where id=@orid)= '是')
        begin
...
class AdminPage:
    def __init__(self, parent_window):
        ...
        self.window.iconbitmap('renyuan.ico')  # 更改窗口图标
        #self.window.geometry('450x300+500+100')
        self.window.geometry('400x250+500+100')
        ...
        # tk.Label(self.window, text='用户密码: ').place(x=80, y=190)
        ...
        # btn_back = Button(self.window, text="返回", width=10, command=self.back)
        # btn_back.place(x=270, y=230)

        tk.Label(self.window, text='用户名: ').place(x=70, y=90)
        tk.Label(self.window, text='用户密码: ').place(x=70, y=120)

        self.admin_username = tk.Entry(self.window)
        self.admin_username.place(x=150, y=90)
        self.admin_pass = tk.Entry(self.window, show='*')
        self.admin_pass.place(x=150, y=120)
        # 登陆和返回首页得按钮
        btn_login = tk.Button(self.window, text='登陆', width=10, command=self.login)
        btn_login.place(x=110, y=170)
        btn_back = Button(self.window, text="返回", width=10, command=self.back)
        btn_back.place(x=260, y=170)
...
class All_admin:
    def __init__(self, parent_window):
        ...
        self.window.iconbitmap('dingdan3.ico')  # 更改窗口图标
        self.window.geometry('500x410+500+100')
        label = Label(self.window, text="请选择需要进行的操作", font=("微软雅黑", 20))
        label.pack(pady=70)  # pady=100 界面的长度

        Button(self.window, text="服装信息管理", font=tkFont.Font(size=16), width=30, height=2,
...
class AdminManage:
    def __init__(self, parent_window):
        ...
        self.window.title('服装信息管理') #管理员操作界面
        self.window.iconbitmap('dingdan3.ico')  # 更改窗口图标
        ...
        # 定义下方中心列表区域
        self.columns = ("服装id", "服装名称", "服装价格", "销售数量")
        ...
        # 表格的标题
        self.tree.column("服装id", width=150, anchor='center')
        self.tree.column("服装名称", width=150, anchor='center')
        self.tree.column("服装价格", width=100, anchor='center')
        ...
        # 打开数据库连接
        db = pymssql.connect('YANPENG', 'sa', 'Peng.0921', 'EMIS')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        ...
        # 定义顶部区域
        # 定义左上方区域
        self.top_title = Label(self.frame_left_top, text="服装信息:", font=('Verdana', 20))
        ...
        self.right_bottom_button = ttk.Button(self.frame_bottom, text='服装名称查询', width=20, command=self.put_data)
        ...
        # 商品id
        self.right_top_id_label = Label(self.frame_left_top, text="服装id  ： ", font=('Verdana', 15))
        ...
        # 商品名称
        self.right_top_name_label = Label(self.frame_left_top, text="服装名称：", font=('Verdana', 15))
        ...
        # 商品价格
        self.right_top_gender_label = Label(self.frame_left_top, text="服装价格：", font=('Verdana', 15))
        ...
        self.right_top_button1 = ttk.Button(self.frame_right_top, text='新建服装信息', width=20, command=self.new_row)
        self.right_top_button2 = ttk.Button(self.frame_right_top, text='更新选中服装信息', width=20, command=self.updata_row)
        self.right_top_button3 = ttk.Button(self.frame_right_top, text='删除选中服装信息', width=20, command=self.del_row)
    ...
    def new_row(self):
        ...
        if str(self.var_id.get()) in self.id:
            ...
            if self.var_id.get() != '' and self.var_name.get() != '' and self.var_gender.get() != '' and self.var_age.get() != '':
                ...
                messagebox.showinfo('警告！', '请填写服装信息')
...
class Buy_AdminManage:
    def __init__(self, parent_window):
        ...
        # 定义下方中心列表区域
        self.columns = ("服装id", "订单id", "订单创建时间", "是否成功交易", "交易金额")
        self.tree = ttk.Treeview(self.frame_center, show="headings", height=18, columns=self.columns)
        self.vbar = ttk.Scrollbar(self.frame_center, orient=VERTICAL, command=self.tree.yview)
        # 定义树形结构与滚动条
        self.tree.configure(yscrollcommand=self.vbar.set)
        ...
        # 表格的标题
        self.tree.column("服装id", width=100, anchor='center')  # 表示列,不显示
    ...
    def new_row(self):
        print(self.var_id.get())
        print(self.SNO)
        if str(self.var_name.get()) in self.SNO:
            messagebox.showinfo('警告！', '该商品已存在！')
        else:
...
class AboutPage:
    def __init__(self, parent_window):
        ...
        label = tk.Label(self.window, text='服装信息管理系统', bg='green', font=('Verdana', 20), width=30, height=2)
        label.pack()

        Label(self.window, text='作者：YANPENG', font=('Verdana', 18)).pack(pady=30)
